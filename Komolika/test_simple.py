#!/usr/bin/env python3
"""
Test script for the simple trained model
"""
...
def main():
    parser = argparse.ArgumentParser(description='Generate caption for an image using simple trained model')
    parser.add_argument('-i', '--image', required=True, help='Path to the image file')
    parser.add_argument('--model', default='models/model_simple.h5', help='Path to the trained model')
    parser.add_argument('--tokenizer', default='models/tokenizer_simple.pkl', help='Path to the tokenizer')
    ...
    print("🖼️  IMAGE CAPTION GENERATION (Simple Model)")
    print("=" * 50)
    ...
    if args.show:
        try:
            ...
            plt.imshow(img)
            plt.title(f"Caption: {description}", fontsize=14, wrap=True)
            plt.axis('off')
            plt.tight_layout()
            plt.show()
        except Exception as e:
